...
const FarmCards: React.FC = () => {
  ...
  const apeIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'APE',
  )

  const apePrice =
    apeIndex >= 0 && stakedValue[apeIndex]
      ? stakedValue[apeIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const APE_PER_BLOCK = new BigNumber(50)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [ape, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'APE' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
